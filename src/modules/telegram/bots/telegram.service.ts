...
export class TelegramService implements OnModuleInit, OnModuleDestroy {
	...
	private runPararBotCommand() {
		this.bot.command("pararbot", async (ctx) => {
			...
			const subscribers = await this.okami.fetchSubscribersByChatId(ctx.chat.id.toString());
			...
			for (const subscriber of subscribers) {
				...
				await ctx.reply(`O bot foi desvinculado do chat do seu usuário ${subscriber.email}`);
			}
		});
	}
	...
	async healthCheck() {
		try {
			...
			return this.indicator.down(error as unknown as Record<string, unknown>);
		}
	}
}
