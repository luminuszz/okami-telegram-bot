...
export class BullQueueProvider implements QueueProvider {
  ...
  subscribe<Payload = any>(
    ...
    if (!existsWorker) {
      ...
      existsWorker.on('completed', (job) =>
        this.logger.log(`Job ${job.id} completed`),
      );
      existsWorker.on('failed', (job, err) =>
        this.logger.error(`Job ${job.id} failed with error: ${err}`),
      );
      existsWorker.on('progress', (job, progress) =>
  ...
  healthCheck() {
    ...
    for (const [key, consumer] of this.workers.entries()) {
      ...
      this.logger.debug(
        `Health check for ${key}: ${consumer.isRunning() ? 'UP' : 'DOWN'}`,
      );
    }
